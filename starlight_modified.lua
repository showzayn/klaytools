...
local function Hide(Interface, JustHide: boolean?, Notify: boolean?, Bind: string?)
	...
	if Starlight.Instance and (Interface == Starlight.Instance or Interface == Starlight.Instance.MainWindow) then
...
if UserInputService.TouchEnabled then
	StarlightUI.Notifications.Active = false
	...
	task.spawn(function()
		...
		if UserInputService.TouchEnabled then
			pcall(function()
				...
				newNotification.Interactable = false
			end)
		end
...
function Starlight:CreateWindow(WindowSettings)
	...
	do
		...
		mainWindow.Content.Topbar.Controls.Close["MouseButton1Click"]:Connect(function()
			...
			local debounce = false
			local function setMainWindowHidden(hidden: boolean, notify: boolean?)
				if Starlight.Minimized == hidden then
					return
				end

				mainWindow.Visible = not hidden
				StarlightUI.Drag.Visible = not hidden
				Starlight.Minimized = hidden

				if hidden and notify then
					if Starlight.Instance.MobileToggle.Visible then
						Starlight:Notification({
							Title = "Interface Hidden",
							Icon = 87575513726659,
						...
						Starlight:Notification({
							Title = "Interface Hidden",
							Icon = 87575513726659,
							Content = "The Interface Has Been Hidden. You May Reopen It By Pressing The " .. Starlight.WindowKeybind .. " Key.  ",
							Duration = 2,
						})
					end
